    ...
    if (exists) {
      ...
      if (response.status === 204) {
        ...
        if (parseInt(amount, 10) === 0) {
          message = "Amount was set to zero, value stays at " + old_value + ".";
        }
        if (parseInt(amount, 10) < 0) {
          message = "Succesfully decremented " + name + " from " + old_value + " to " + new_value + ".";
        }
        if (parseInt(amount, 10) > 0) {
          message = "Succesfully incremented " + name + " from " + old_value + " to " + new_value + ".";
        }
      ...
      if (response.status === 201) {
        return {
          old_value: "",
          new_value: amount,
          message: "Succesfully created variable " + name + " with value " + amount + "."
        };
      }
...
bootstrap()
  .then(
    (result) => {
      core.setOutput("old_value", result.old_value);
      core.setOutput("new_value", result.new_value);
